...
def recover_all_poses_from_homography(
...
def select_best_solution(
    ...
    Args:
        solutions: List of pose candidates from homography decomposition. Each solution is a
            tuple (R, t, n) representing rotation, translation, and plane normal respectively.
        expected_z_positive: Whether to enforce positive Z translation (plane in front
            of camera). Set to False for scenarios where the plane might be behind the camera.
...
def derive_metric_homography(
    ...
    Creates a composite transformation that enables direct mapping from metric coordinates
    on a template to pixel coordinates in a scene image by combining coordinate system
    transformations with an existing pixel-to-pixel homography.

    Args:
        H_px: 3x3 homography matrix mapping template pixel coordinates to scene pixels.
        template_size_px: Template dimensions in pixels as (height, width).
        template_size_metric: Template dimensions in metric units as (height, width).
        template_origin_px: Pixel coordinates (x, y) corresponding to metric origin.
        template_origin_metric: Metric coordinates (x, y) defining the origin.

    Returns:
        3x3 homography matrix mapping metric coordinates [X_m, Y_m, 1]^T to
        scene image pixel coordinates [u_img, v_img, 1]^T.

    Raises:
        ValueError: If inputs are invalid or result in ill-conditioned homography.
    """
    # Input validation
    if not isinstance(H_px, np.ndarray) or H_px.shape != (3, 3):
        raise ValueError(
    ...
    # Create composite pixel-to-metric transformation
    M_px_to_metric = np.array(
        [
            [s_x, 0.0, u0_px - s_x * X0_m],
            [0.0, s_y, v0_px - s_y * Y0_m],
            [0.0, 0.0, 1.0],
        ],
        dtype=np.float64,
    )

    # Apply template-to-scene homography
    H_metric = H_px @ M_px_to_metric
...
def compute_distance_from_pose(
    R: np.ndarray, t: np.ndarray, p_metric: tuple[float, float]
) -> float:
    """
    Compute the Euclidean distance from camera center to a 3D point on the template plane.

    Given the camera pose (rotation and translation) relative to the template plane,
    computes the 3D distance to any point on that plane specified in metric coordinates.

    Args:
        R: 3x3 rotation matrix representing the orientation of the template
            plane relative to the camera coordinate system.
        t: 3D translation vector from camera center to the template
            plane origin, in the same units as p_metric.
        p_metric: 2D coordinates (X_metric, Y_metric) of the target point on the
            template plane, expressed in metric units.

    Returns:
        Euclidean distance from the camera center to the specified 3D point,
        expressed in the same units as the input coordinates.
    """
    # Extract 2D coordinates
    X_metric, Y_metric = p_metric

    # Extract rotation column vectors (first two columns represent the plane's basis vectors)
    r1 = R[:, 0]
    r2 = R[:, 1]

    # Compute 3D point coordinates in camera reference frame
    P = X_metric * r1 + Y_metric * r2 + t

    # Calculate Euclidean distance from camera center
    return np.linalg.norm(P)


def create_template_corners(width: float, height: float) -> np.ndarray:
    """
    Create template corner points in world coordinates (Z=0 plane).
    ...
    Returns:
        4x3 array of corner coordinates: [top-left, top-right, bottom-right, bottom-left].
    """
    return np.array(
        [
            [0, 0, 0],
...
def transform_to_camera(
    points_world: np.ndarray, R: np.ndarray, t: np.ndarray
) -> np.ndarray:
    """
    Transform points from world to camera coordinates.

    Applies R @ points + t, then applies 180° rotation around Z-axis
    to match OpenCV convention (flip X and Y).
    ...
    # OpenCV convention: flip X and Y
    points_cam[:, 0] *= -1
    points_cam[:, 1] *= -1
